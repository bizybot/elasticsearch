...
public class ReservedRolesStoreTests extends ESTestCase {
    ...
    public void testSnapshotUserRole() {
        ...
        assertThat(snapshotUserRole.cluster().check(GetRepositoriesAction.NAME, request, authn), is(true));
        assertThat(snapshotUserRole.cluster().check(CreateSnapshotAction.NAME, request, authn), is(true));
        assertThat(snapshotUserRole.cluster().check(SnapshotsStatusAction.NAME, request, authn), is(true));
        assertThat(snapshotUserRole.cluster().check(GetSnapshotsAction.NAME, request, authn), is(true));

        assertThat(snapshotUserRole.cluster().check(PutRepositoryAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(GetIndexTemplatesAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(DeleteIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(PutPipelineAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(GetPipelineAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(DeletePipelineAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(GetWatchAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(PutWatchAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(DeleteWatchAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(ExecuteWatchAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(AckWatchAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(ActivateWatchAction.NAME, request, authn), is(false));
        assertThat(snapshotUserRole.cluster().check(WatcherServiceAction.NAME, request, authn), is(false));
    ...
    public void testIngestAdminRole() {
        ...
        assertThat(ingestAdminRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(true));
        assertThat(ingestAdminRole.cluster().check(GetIndexTemplatesAction.NAME, request, authn), is(true));
        assertThat(ingestAdminRole.cluster().check(DeleteIndexTemplateAction.NAME, request, authn), is(true));
        assertThat(ingestAdminRole.cluster().check(PutPipelineAction.NAME, request, authn), is(true));
        assertThat(ingestAdminRole.cluster().check(GetPipelineAction.NAME, request, authn), is(true));
        assertThat(ingestAdminRole.cluster().check(DeletePipelineAction.NAME, request, authn), is(true));

        assertThat(ingestAdminRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(ingestAdminRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(ingestAdminRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
    ...
    public void testKibanaSystemRole() {
        ...
        assertThat(kibanaRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(true));
        assertThat(kibanaRole.cluster().check(ClusterStateAction.NAME, request, authn), is(true));
        assertThat(kibanaRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(true));
        assertThat(kibanaRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(true));
        assertThat(kibanaRole.cluster().check(GetIndexTemplatesAction.NAME, request, authn), is(true));
        assertThat(kibanaRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(kibanaRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(kibanaRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(true));

        // SAML and token
        assertThat(kibanaRole.cluster().check(SamlPrepareAuthenticationAction.NAME, request, authn), is(true));
        assertThat(kibanaRole.cluster().check(SamlAuthenticateAction.NAME, request, authn), is(true));
        assertThat(kibanaRole.cluster().check(InvalidateTokenAction.NAME, request, authn), is(true));
        assertThat(kibanaRole.cluster().check(CreateTokenAction.NAME, request, authn), is(true));
        ...
        assertThat(kibanaRole.cluster().check(DeletePrivilegesAction.NAME, deleteKibanaPrivileges, authn), is(true));
        assertThat(kibanaRole.cluster().check(DeletePrivilegesAction.NAME, deleteLogstashPrivileges, authn), is(false));
        ...
        assertThat(kibanaRole.cluster().check(GetPrivilegesAction.NAME, getKibanaPrivileges, authn), is(true));
        assertThat(kibanaRole.cluster().check(GetPrivilegesAction.NAME, getApmPrivileges, authn), is(false));
        ...
        assertThat(kibanaRole.cluster().check(PutPrivilegesAction.NAME, putKibanaPrivileges, authn), is(true));
        assertThat(kibanaRole.cluster().check(PutPrivilegesAction.NAME, putSwiftypePrivileges, authn), is(false));

        assertThat(kibanaRole.cluster().check(GetBuiltinPrivilegesAction.NAME, request, authn), is(true));
    ...
    public void testKibanaUserRole() {
        ...
        assertThat(kibanaUserRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(false));
        assertThat(kibanaUserRole.cluster().check(ClusterStateAction.NAME, request, authn), is(false));
        assertThat(kibanaUserRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(false));
        assertThat(kibanaUserRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(kibanaUserRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(kibanaUserRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(kibanaUserRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
    ...
    public void testMonitoringUserRole() {
        ...
        assertThat(monitoringUserRole.cluster().check(MainAction.NAME, request, authn), is(true));
        assertThat(monitoringUserRole.cluster().check(XPackInfoAction.NAME, request, authn), is(true));
        assertThat(monitoringUserRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(false));
        assertThat(monitoringUserRole.cluster().check(ClusterStateAction.NAME, request, authn), is(false));
        assertThat(monitoringUserRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(false));
        assertThat(monitoringUserRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(monitoringUserRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(monitoringUserRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(monitoringUserRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
    ...
    public void testRemoteMonitoringAgentRole() {
        ...
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterStateAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(GetWatchAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(PutWatchAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(DeleteWatchAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(ExecuteWatchAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(AckWatchAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(ActivateWatchAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(WatcherServiceAction.NAME, request, authn), is(false));
        // we get this from the cluster:monitor privilege
        assertThat(remoteMonitoringAgentRole.cluster().check(WatcherStatsAction.NAME, request, authn), is(true));
    ...
    public void testRemoteMonitoringCollectorRole() {
        ...
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterStateAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(true));
        assertThat(remoteMonitoringAgentRole.cluster().check(GetIndexTemplatesAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(DeleteIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(remoteMonitoringAgentRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
    ...
    public void testReportingUserRole() {
        ...
        assertThat(reportingUserRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(false));
        assertThat(reportingUserRole.cluster().check(ClusterStateAction.NAME, request, authn), is(false));
        assertThat(reportingUserRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(false));
        assertThat(reportingUserRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(reportingUserRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(reportingUserRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(reportingUserRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
    ...
    public void testKibanaDashboardOnlyUserRole() {
        ...
        assertThat(dashboardsOnlyUserRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(false));
        assertThat(dashboardsOnlyUserRole.cluster().check(ClusterStateAction.NAME, request, authn), is(false));
        assertThat(dashboardsOnlyUserRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(false));
        assertThat(dashboardsOnlyUserRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(dashboardsOnlyUserRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(dashboardsOnlyUserRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(dashboardsOnlyUserRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
    ...
    public void testSuperuserRole() {
        ...
        assertThat(superuserRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(true));
        assertThat(superuserRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(true));
        assertThat(superuserRole.cluster().check(PutUserAction.NAME, request, authn), is(true));
        assertThat(superuserRole.cluster().check(PutRoleAction.NAME, request, authn), is(true));
        assertThat(superuserRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(true));
        assertThat(superuserRole.cluster().check("internal:admin/foo", request, authn), is(false));
    ...
    public void testLogstashSystemRole() {
        ...
        assertThat(logstashSystemRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(true));
        assertThat(logstashSystemRole.cluster().check(ClusterStateAction.NAME, request, authn), is(true));
        assertThat(logstashSystemRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(true));
        assertThat(logstashSystemRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(logstashSystemRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(logstashSystemRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(logstashSystemRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(true));
    ...
    public void testBeatsAdminRole() {
        ...
        assertThat(beatsAdminRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(false));
        assertThat(beatsAdminRole.cluster().check(ClusterStateAction.NAME, request, authn), is(false));
        assertThat(beatsAdminRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(false));
        assertThat(beatsAdminRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(beatsAdminRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(beatsAdminRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(beatsAdminRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(false));
    ...
    public void testBeatsSystemRole() {
        ...
        assertThat(beatsSystemRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(true));
        assertThat(beatsSystemRole.cluster().check(ClusterStateAction.NAME, request, authn), is(true));
        assertThat(beatsSystemRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(true));
        assertThat(beatsSystemRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(beatsSystemRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(beatsSystemRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(beatsSystemRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(true));
    ...
    public void testAPMSystemRole() {
        ...
        assertThat(APMSystemRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(true));
        assertThat(APMSystemRole.cluster().check(ClusterStateAction.NAME, request, authn), is(true));
        assertThat(APMSystemRole.cluster().check(ClusterStatsAction.NAME, request, authn), is(true));
        assertThat(APMSystemRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(APMSystemRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(APMSystemRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
        assertThat(APMSystemRole.cluster().check(MonitoringBulkAction.NAME, request, authn), is(true));
    ...
    public void testMachineLearningAdminRole() {
        ...
        Role role = Role.builder(roleDescriptor, null).build();
        assertThat(role.cluster().check(CloseJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteCalendarAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteCalendarEventAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteDatafeedAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteExpiredDataAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteFilterAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteForecastAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteModelSnapshotAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(FinalizeJobExecutionAction.NAME, request, authn), is(false)); // internal use only
        assertThat(role.cluster().check(FindFileStructureAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(FlushJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(ForecastJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetBucketsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetCalendarEventsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetCalendarsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetCategoriesAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetDatafeedsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetDatafeedsStatsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetFiltersAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetInfluencersAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetJobsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetJobsStatsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetModelSnapshotsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetOverallBucketsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetRecordsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(IsolateDatafeedAction.NAME, request, authn), is(false)); // internal use only
        assertThat(role.cluster().check(KillProcessAction.NAME, request, authn), is(false)); // internal use only
        assertThat(role.cluster().check(MlInfoAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(OpenJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PersistJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PostCalendarEventsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PostDataAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PreviewDatafeedAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PutCalendarAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PutDatafeedAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PutFilterAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PutJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(RevertModelSnapshotAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(SetUpgradeModeAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(StartDatafeedAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(StopDatafeedAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(UpdateCalendarJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(UpdateDatafeedAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(UpdateFilterAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(UpdateJobAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(UpdateModelSnapshotAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(UpdateProcessAction.NAME, request, authn), is(false)); // internal use only
        assertThat(role.cluster().check(ValidateDetectorAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(ValidateJobConfigAction.NAME, request, authn), is(true));
    ...
    public void testMachineLearningUserRole() {
        ...
        Role role = Role.builder(roleDescriptor, null).build();
        assertThat(role.cluster().check(CloseJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(DeleteCalendarAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(DeleteCalendarEventAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(DeleteDatafeedAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(DeleteExpiredDataAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(DeleteFilterAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(DeleteForecastAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(DeleteJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(DeleteModelSnapshotAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(FinalizeJobExecutionAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(FindFileStructureAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(FlushJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(ForecastJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(GetBucketsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetCalendarEventsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetCalendarsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetCategoriesAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetDatafeedsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetDatafeedsStatsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetFiltersAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(GetInfluencersAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetJobsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetJobsStatsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetModelSnapshotsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetOverallBucketsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetRecordsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(IsolateDatafeedAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(KillProcessAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(MlInfoAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(OpenJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PersistJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PostCalendarEventsAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PostDataAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PreviewDatafeedAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PutCalendarAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PutDatafeedAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PutFilterAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PutJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(RevertModelSnapshotAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(SetUpgradeModeAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(StartDatafeedAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(StopDatafeedAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(UpdateCalendarJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(UpdateDatafeedAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(UpdateFilterAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(UpdateJobAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(UpdateModelSnapshotAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(UpdateProcessAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(ValidateDetectorAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(ValidateJobConfigAction.NAME, request, authn), is(false));
        assertThat(role.runAs().check(randomAlphaOfLengthBetween(1, 30)), is(false));
    ...
    public void testDataFrameTransformsAdminRole() {
        ...
        Role role = Role.builder(roleDescriptor, null).build();
        assertThat(role.cluster().check(DeleteDataFrameTransformAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetDataFrameTransformsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetDataFrameTransformsStatsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PreviewDataFrameTransformAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PutDataFrameTransformAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(StartDataFrameTransformAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(StopDataFrameTransformAction.NAME, request, authn), is(true));
        assertThat(role.runAs().check(randomAlphaOfLengthBetween(1, 30)), is(false));
    ...
    public void testDataFrameTransformsUserRole() {
        ...
        Role role = Role.builder(roleDescriptor, null).build();
        assertThat(role.cluster().check(DeleteDataFrameTransformAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(GetDataFrameTransformsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetDataFrameTransformsStatsAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(PreviewDataFrameTransformAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(PutDataFrameTransformAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(StartDataFrameTransformAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(StopDataFrameTransformAction.NAME, request, authn), is(false));
        assertThat(role.runAs().check(randomAlphaOfLengthBetween(1, 30)), is(false));
    ...
    public void testWatcherAdminRole() {
        ...
        Role role = Role.builder(roleDescriptor, null).build();
        assertThat(role.cluster().check(PutWatchAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(GetWatchAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteWatchAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(ExecuteWatchAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(AckWatchAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(ActivateWatchAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(WatcherServiceAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(WatcherStatsAction.NAME, request, authn), is(true));
        assertThat(role.runAs().check(randomAlphaOfLengthBetween(1, 30)), is(false));
    ...
    public void testWatcherUserRole() {
        ...
        Role role = Role.builder(roleDescriptor, null).build();
        assertThat(role.cluster().check(PutWatchAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(GetWatchAction.NAME, request, authn), is(true));
        assertThat(role.cluster().check(DeleteWatchAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(ExecuteWatchAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(AckWatchAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(ActivateWatchAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(WatcherServiceAction.NAME, request, authn), is(false));
        assertThat(role.cluster().check(WatcherStatsAction.NAME, request, authn), is(true));
        assertThat(role.runAs().check(randomAlphaOfLengthBetween(1, 30)), is(false));
    ...
    public void testLogstashAdminRole() {
        ...
        assertThat(logstashAdminRole.cluster().check(ClusterHealthAction.NAME, request, authn), is(false));
        assertThat(logstashAdminRole.cluster().check(PutIndexTemplateAction.NAME, request, authn), is(false));
        assertThat(logstashAdminRole.cluster().check(ClusterRerouteAction.NAME, request, authn), is(false));
        assertThat(logstashAdminRole.cluster().check(ClusterUpdateSettingsAction.NAME, request, authn), is(false));
